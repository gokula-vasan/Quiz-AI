...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.detail || 'Invalid email/username or password. Please try again.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-[calc(100vh-100px)] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md bg-[#0e1626]/60 border border-slate-800 rounded-2xl p-8 backdrop-blur-md shadow-2xl relative overflow-hidden">
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <p className="text-sm text-slate-400 mt-2">Sign in to continue your learning journey</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
          <div>
            <label className="block text-sm font-semibold text-slate-300 mb-2">Email Address or Username</label>
